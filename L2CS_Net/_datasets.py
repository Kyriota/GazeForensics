...
class Gaze360(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ''' Testing
        # save img before transform
        img.save("_before.jpg")
        '''

        if self.transform:
            img = [self.transform[0](img), self.transform[1](img)]
        
        ''' Testing
        # save img after transform
        img[0] = img[0].squeeze(0).numpy().transpose(1, 2, 0)
        img[0] = img[0].astype(np.float32)
        img[0] -= img[0].min()
        img[0] /= img[0].max()
        img[0] *= 255
        img[0] = img[0].astype(np.uint8)
        img[0] = Image.fromarray(img[0])
        img[0].save("_after0.jpg")
        
        img[1] = img[1].squeeze(0).numpy().transpose(1, 2, 0)
        img[1] = img[1].astype(np.float32)
        img[1] -= img[1].min()
        img[1] /= img[1].max()
        img[1] *= 255
        img[1] = img[1].astype(np.uint8)
        img[1] = Image.fromarray(img[1])
        img[1].save("_after1.jpg")
