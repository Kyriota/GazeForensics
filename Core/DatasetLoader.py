...
class DatasetLoader(TDM.Dataset):
    ...
    def get_normalization(self):
        ...
        for i in range(len(self.dataset)):
            ...
            for _ in range(14):
                _, image = vidcap.read()
                image = (image[:, :, ::-1] /
                         255.0).astype(np.float64)  # BGR to RGB
                # Calculate channel sum and channel squared sum
                temp_channels_sum.append(np.sum(image, axis=(0, 1)))
                temp_channels_sqrd_sum.append(
                    np.sum(np.square(image), axis=(0, 1)))
            ...
            if i % 100 == 0 or i == len(self.dataset) - 1:
                print('Progress: {}/{}'.format(i+1, len(self.dataset)),
                      end='\r' if i != len(self.dataset) - 1 else '\n')
    ...
    def __getitem__(self, index):
        ...
        if self.use_argument:
            inputTensor = {
                TransformType.standard: torch.FloatTensor(14, 3, 224, 224),
                TransformType.argument: torch.FloatTensor(14, 3, 224, 224),
            }
            for i in range(14):
                _, image = vidcap.read()
                ...
                inputTensor[TransformType.standard][i] = self.transform[TransformType.standard](
                    image)
                inputTensor[TransformType.argument][i] = self.transform[TransformType.argument](
                    image)
