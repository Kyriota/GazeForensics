...
class EvaluateManager:
        ...
        def get_shrinked_dataset():
            ...
            for i in data_dir_count:
                if i[1] == i[2]:
                    ...
                    #  then use all clips
                    temp_dir_list = [[
                        0 if 'fake' in i[0] else 1, config.test.testClipsDir + i[0] + '_' + str(j) + '.mp4'
                    ] for j in range(i[2])]
                else:
    ...
    def evaluate(
        ...
        def single_shot(pth_path, epoch=1, epochs=1):
            ...
            with torch.no_grad():
                for sign, frames, vid_paths in self.data_loader:
                    ...
                    _frames = frames[TransformType.standard].cuda(non_blocking=True)
            ...
            vid_auc = roc_auc_score(vid_ground_truths, vid_preds)
            ...
            self.eval_print(' > Vid Acc: {:.4f} | Seq Acc: {:.4f} | Loss: {:.4f} | AUC: {:.4f}'.format(
                vid_acc, acc, loss, vid_auc
            ))

            return vid_acc, acc, loss, confusion_mat, list(zip(vid_ids, vid_corrects, vid_preds)), list(zip(ids, corrects, preds)), vid_auc
        ...
        if checkpoint_path.endswith('.pth'):
            ...
            return vid_acc, acc, loss, detail_vid, detail_seq, vid_auc
        ...
        for epoch, pth_path in enumerate(checkpoint_paths):
            vid_acc, acc, loss, confusion_mat, _, _, vid_auc = single_shot(
            ...
            vid_aucs.append(vid_auc)

        result = {
            'test_result': {'vid_acc': vid_accs, 'acc': accs, 'out_loss': losses, 'confusion_mat': confusion_mats, 'vid_auc': vid_aucs},
            'train_result': get_train_result(checkpoint_path + checkpoint_paths[-1]),
        }
        with open(self.config.test.resultDir + self.config.basic.evalID + '_result.json', 'w') as f:
            json.dump(result, f)
        if self.config.test.onlyEvalLastN is None:
            PlotResult(
                result,
                result_path=self.config.test.resultDir + self.config.basic.evalID + '.png'
            )
