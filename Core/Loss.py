...
class TrainLoss:
    def __init__(self, params_loss):
        ...
        self.decoder_weight = params_loss['decoder_weight']
    ...
    def __call__(self, output, target):
        ...
        self.out_loss = self.BCE(output['out'][:, :2], target['out'])
        loss = self.out_loss

        if self.gaze_weight > 0.0:
            self.gaze_loss = self.MSE(output['gaze'], target['gaze'])
            loss = loss + self.gaze_weight * self.gaze_loss
        
        if self.enable_decoder:
            self.decoder_loss = self.MSE(output['decoder'], target['decoder'])
            loss = loss - self.decoder_loss * self.decoder_weight
