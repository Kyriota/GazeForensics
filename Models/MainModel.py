...
class GazeForensics(nn.Module):
    def __init__(
        ...
        if self.freeze_backend:
            assert type(gaze_backend) is nn.DataParallel and type(gaze_backend.module) is GazeBackend
        ...
        if mid_sizes['gaze_fc'] is None:
            mid_sizes['gaze_fc'] = [self.total_emb_dim]
        if mid_sizes['MHA_fc'] is None:
            mid_sizes['MHA_fc'] = []
        if mid_sizes['MHA_comp'] is None:
            mid_sizes['MHA_comp'] = [128]
        if mid_sizes['last_fc'] is None:
            mid_sizes['last_fc'] = [
        ...
        if not self.freeze_backend or self.leaky_dim > 0:
            ...
            if not self.freeze_backend:
                self.gaze_fc = FC_block(self.avg_dim, self.total_emb_dim, mid_sizes=mid_sizes['gaze_fc'])
            else: # frozen backend, but leaky > 0
                self.gaze_fc = FC_block(self.avg_dim, self.leaky_dim, mid_sizes=mid_sizes['gaze_fc'])

        self.MHA_Q_fc = FC_block(self.total_emb_dim, self.MHA_dim, mid_sizes=mid_sizes['MHA_fc'])
        self.MHA_K_fc = FC_block(self.total_emb_dim, self.MHA_dim, mid_sizes=mid_sizes['MHA_fc'])
        self.MHA_V_fc = FC_block(self.total_emb_dim, self.MHA_dim, mid_sizes=mid_sizes['MHA_fc'])
        self.multihead_attn = nn.MultiheadAttention(self.MHA_dim, head_num, batch_first=True)
        self.MHA_comp = FC_block(self.MHA_dim, self.MHA_comp_dim, mid_sizes=mid_sizes['MHA_comp'])
        self.last_fc = FC_block(last_fc_input_size, 2, mid_sizes=mid_sizes['last_fc'])


    def forward(self, x):
        batch_size = x.size(0)
        if self.freeze_backend:
            frozen_out = self.gaze_backend(x.view(-1, 3, 224, 224)).view(batch_size, self.fpc, self.gaze_emb_dim)
        if not self.freeze_backend or self.leaky_dim > 0:
        ...
        if self.freeze_backend:
            x = frozen_out if self.leaky_dim == 0 else torch.cat((x, frozen_out), dim=2)
